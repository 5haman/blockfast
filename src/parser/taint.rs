...
use blockchain::proto::tx::{EvaluatedTxOut, Tx, TxInput, TxOutpoint, TxOutput};
...
use blockchain::proto::script::{
    eval_from_bytes, eval_from_stack, ScriptError, ScriptEvaluator, ScriptPattern,
};
...
impl fmt::Display for AddressInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let yo: String = if let Some(ref x) = self.tainted_balance {
            x.iter().fold(String::new(), |acc, k| {
                acc + &k.name.to_string() + " " + &k.value.to_string() + ","
            })
        } else {
            String::from("")
        };

        write!(f, "{},{},{}", self.timestamp, self.current_balance, yo)
    }
}
...
fn count_fragments(am: &HashMap<TxOutpoint, AddressInfo, BuildHasherDefault<XxHash>>) -> usize {
    return am.iter().fold(0, |a, v| {
        a + if let Some(ref x) = v.1.tainted_balance {
            x.len()
        } else {
            0
        }
...
fn count_accounts(am: &HashMap<TxOutpoint, AddressInfo, BuildHasherDefault<XxHash>>) -> usize {
    return am.iter().fold(0, |a, v| {
        a + if let Some(ref _x) = v.1.tainted_balance {
            1
        } else {
            0
        }
...
fn non_white_taint_to_sum(vdtp: &VecDeque<TaintPart>) -> u64 {
    return vdtp
        .iter()
        .fold(0, |acc, k| acc + if k.name != 0 { k.value } else { 0 });
}

fn taint_to_string(vdtp: &Option<VecDeque<TaintPart>>) -> String {
    if let &Some(ref x) = vdtp {
        return x.iter().fold(String::new(), |a, k| {
            a + &k.name.to_string() + " " + &k.value.to_string() + ","
        });
    }
    return String::from("");
}
...
fn inputs_to_string(vdtp: &Hashed<Tx>) -> String {
    return format!(
        "{},{}",
        vdtp.value.inputs.len(),
        vdtp.value
            .inputs
            .iter()
            .fold(String::new(), |a, k| { a + &k.outpoint.to_string() + "," })
    );
}
...
fn extract_taint(given_taints: &mut VecDeque<TaintPart>, value: u64) -> VecDeque<TaintPart> {
    ...
    while remaining > 0 {
        if given_taints.is_empty() {
            new_tainted_balance.push_back(TaintPart {
                name: 0,
                value: remaining,
            });
            assert!(remaining != 0);
...
fn combine_taints(
    taints: &mut VecDeque<TaintPart>,
    tainted_balance: &mut VecDeque<TaintPart>,
    mut writer_to_notify: Option<&mut LineWriter<File>>,
                ...
                if let Some(ref mut x) = writer_to_notify {
                    x.write_all(
                        format!(
                            ...
                            left.name,
                            right.name,
                            cmp::min(left.value, right.value),
                            collision_name,
                            location
                        )
                        .as_bytes(),
...
fn reduce_taint(tainted_balance: &mut VecDeque<TaintPart>) {
    ...
    for elem in hru.iter() {
        if elem.name == lname {
            ...
            if vl != 0 {
                tainted_balance.push_back(TaintPart {
                    name: lname,
                    value: vl,
                });
            }
...
impl TaintFifo {
    ...
    fn rename_tmp_files(&mut self) -> OpResult<usize> {
        fs::rename(
            self.dump_folder.as_path().join("taint_mapper.csv.tmp"),
            self.dump_folder.as_path().join("taint_mapper.csv"),
        )
        .expect("Unable to rename taint mapping file");
        fs::rename(
            self.dump_folder.as_path().join("taint_overlap.csv.tmp"),
        ...
        fs::rename(
            self.dump_folder
                .as_path()
                .join("taint_timing_information.csv.tmp"),
            self.dump_folder
                .as_path()
                .join("taint_timing_information.csv"),
        )
        .expect("Unable to rename taint_clusters.csv.tmp file!");
        fs::rename(
            self.dump_folder.as_path().join("taint_utxo.csv.tmp"),
            self.dump_folder.as_path().join("taint_utxo.csv"),
        )
        .expect("Unable to rename taint_utxo.csv.tmp file!");
        fs::rename(
            self.dump_folder.as_path().join("address_info.csv.tmp"),
...
impl Callback for TaintFifo {
        ...
        Self: Sized,
    {
        ...
        match (|| -> OpResult<Self> {
            let cb = TaintFifo {
                ...
                taint_mapping_writer: try!(TaintFifo::create_writer(
                    dump_folder.join("taint_mapper.csv.tmp")
                )),
                address_info_writer: try!(TaintFifo::create_writer(
                    dump_folder.join("address_info.csv.tmp")
                )),
                utxo_writer: try!(TaintFifo::create_writer(
                    dump_folder.join("taint_utxo.csv.tmp")
                )),
                overlap_writer: try!(TaintFifo::create_writer(
                    dump_folder.join("taint_overlap.csv.tmp")
                )),
    ...
    fn on_start(&mut self, _: CoinType, block_height: usize) {
        ...
        for line in BufReader::new(file).lines() {
            ...
            if !self.bootstrap_addresses.contains_key(addr) {
                ...
                for s in spt {
                    if tag_v == None {
                        ...
                        if self.dirtmapper.contains_key(&tag) {
                            hmt.push_back(TaintPart {
                                name: *self.dirtmapper.get(&tag).unwrap(),
                ...
                debug!(
                    "Address {} is already in the map. Duplicate detected!",
                    addr
                );
            }
        }
        ...
        for (tag, mapto) in self.dirtmapper.iter() {
            self.taint_mapping_writer
                .write_all(format!("{},{}\n", tag, mapto.to_string()).as_bytes())
                .unwrap();
        }
    }
    ...
    fn on_block(&mut self, block: Block, block_height: usize) {
        ...
        for tx in block.txs.iter() {
            ...
            if tx.value.is_coinbase() {
                for (i, output) in tx.value.outputs.iter().enumerate() {
                    ...
                    let miner_outpoint = TxOutpoint {
                        txid: tx.hash,
                        index: i as u32,
                    };
                    self.utxo_set
                        .insert(miner_outpoint, output.script.address.to_owned());
                }
            }
        }
        ...
        for (tx_index, tx) in block.txs.iter().enumerate() {
            debug!(
                "Tx: {}[I:{} O:{}]",
                arr_to_hex_swapped(&tx.hash),
                tx.value.inputs.len(),
                tx.value.outputs.len()
            );
            if tx.value.is_coinbase() {
                continue;
            }
            ...
            if self.bootstrap_addresses.len() > 0 {
                    ...
                    if assigned_dirt.len() > 0 {
                        for (i, output) in tx.value.outputs.iter().enumerate() {
                            ...
                            let address_info = AddressInfo {
                                ...
                                current_balance: 0,
                                tainted_balance: Some(extract_taint(
                                    assigned_dirt,
                                    output.out.value,
                                )),
                            };
                            ...
                            assert_eq!(
                                taint_to_sum(&address_info.tainted_balance),
                                output.out.value
                            );
                            debug!(
                                "Loading taint for {} : {}",
                                address,
                                taint_to_string(&address_info.tainted_balance)
                            );
                ...
                if to_del {
                    self.bootstrap_addresses.remove(h);
                }
            }
            ...
            assert!(
                self.total_taint_for_tx.is_empty(),
                "A check to see that no taint is being lost from the previous transaction[{}]:{}",
                self.total_taint_for_tx.len(),
                taint_to_string(&Some(self.total_taint_for_tx.clone()))
            );
                ...
                match self.address_mapping.get_mut(&tx_outpoint) {
                    Some(address_info) => {
                        ...
                        debug!(
                            "\t{} INP: {} | BAL: {} | T: {} | SUM: {}",
                            i,
                            txo_to_string(&tx_outpoint, &self.utxo_set),
                            address_info.current_balance,
                            taint_to_string(&address_info.tainted_balance),
                            inputs_summed
                        );
                        ...
                        if let Some(ref x) = address_info.tainted_balance {
                            ...
                            if address_info.current_balance > 0 {
                                self.total_taint_for_tx.push_back(TaintPart {
                                    name: 0,
                                    value: address_info.current_balance,
                                });
                            }
                        }
                        if nullify {
            ...
            assert!(
                inputs_summed >= outputs_summed,
                "Sum of inp {}; Sum of out {}",
                inputs_summed,
                outputs_summed
            );
            ...
            let mut hul: Vec<(EvaluatedTxOut, TxOutpoint, u64)> = tx
                .value
                .outputs
                .iter()
                .enumerate()
            ...
            if miner_fee > 0 {
                ...
                while left_to_payoff > 0 {
                    assert!(
                        !miner_output_queue.is_empty(),
                        "Oops, no more miners left to pay off, but we still need to pay off {}",
                        left_to_payoff
                    );
                    ...
                    if val > left_to_payoff {
                        etx = Some(EvaluatedTxOut {
                            script: cout.script.clone(),
                            out: TxOutput {
                                value: left_to_payoff,
                                script_len: cout.out.script_len.clone(),
                                script_pubkey: cout.out.script_pubkey.clone(),
                            },
                        });
            ...
            for (output, tx_outpoint, taint_offset) in hul {
                ...
                assert!(
                    output.out.value <= (outputs_summed + miner_fee),
                    "output.out.value <= outputs_summed+miner_fee | {}<={}",
                    output.out.value,
                    outputs_summed
                );

                let mut tainted_balance: VecDeque<TaintPart> =
                    extract_taint(&mut self.total_taint_for_tx, output.out.value);
                assert_eq!(
                    taint_to_sum(&Some(tainted_balance.clone())),
                    output.out.value
                );
                ...
                match self.address_mapping.get_mut(&tx_outpoint) {
                    Some(address_info) => {
                        ...
                        if let Some(ref mut x) = address_info.tainted_balance {
                            if taint_offset > 0 {
                                tainted_balance.push_front(TaintPart {
                                    name: 0,
                                    value: taint_offset,
                                });
                            }
                            self.taint_collisions += combine_taints(
                                &mut tainted_balance,
                                x,
                                Some(&mut self.overlap_writer),
                                &arr_to_hex_swapped(&tx.hash),
                            );
                        } else {
                            tainted_balance.push_front(TaintPart {
                ...
                let info = AddressInfo {
                    ...
                    tainted_balance: if !tainted_balance.is_empty() {
                        Some(tainted_balance.clone())
                    } else {
                        None
                    },
                };
                ...
                if taint_offset > 0 {
                    debug!(
                        ...
                        output.out.value,
                        taint_offset,
                        taint_to_string(&info.tainted_balance)
                    );
                } else {
                    debug!(
                        "\t      OUT: {} | VAL: {} | TO: {} | T: {}",
            ...
            assert!(
                tx_inp_taint_sum <= tx_out_taint_sum,
                "Sum(inp_taint) <= Sum(output_taint) | {} <= {}",
                tx_inp_taint_sum,
                tx_out_taint_sum
            );
        ...
        while !miner_output_queue.is_empty() {
            ...
            match self.address_mapping.get_mut(&ctxo) {
                Some(address_info) => {
                    ...
                    debug!(
                        "Miner found {} has new balance of {}",
                        txo_to_string(&ctxo, &self.utxo_set),
                        address_info.current_balance
                    );
            ...
            if !found {
                ...
                debug!(
                    "Miner not found {} has new balance of {}",
                    txo_to_string(&ctxo, &self.utxo_set),
